d3 = function() {
  ...
  d3.geom.hull = function(vertices) {
    ...
    function hull(data) {
      ...
      stack.push(h);
      for (i = 0, j = 0; i < 2; ++j) {
        if (points[j].index_ > -1) {
          stack.push(points[j].index_);
          i++;
        }
      }
      sp = stack.length;
      for (;j < plen; ++j) {
        if (points[j].index_ < 0) continue;
        while (!d3_geom_hullCCW(stack[sp - 2], stack[sp - 1], points[j].index_, vertices)) {
          --sp;
        }
        stack[sp++] = points[j].index_;
      }
      var poly = [];
  ...
  d3.geom.delaunay = function(vertices) {
    ...
    d3_geom_voronoiTessellate(vertices, function(e) {
      edges[e.region.l.index_].push(vertices[e.region.r.index_]);
    });
  ...
  d3.geom.voronoi = function(points) {
    ...
    function voronoi(data) {
      ...
      d3_geom_voronoiTessellate(points, function(e) {
        ...
        polygons[e.region.l.index_].push(v1, v2);
        polygons[e.region.r.index_].push(v1, v2);
      });
    ...
    voronoi.links = function(data) {
      ...
      d3_geom_voronoiTessellate(points, function(e) {
        var l = e.region.l.index_, r = e.region.r.index_;
        if (graph[l][r]) return;
    ...
    force.start = function() {
      ...
      function neighbor() {
        if (!neighbors) {
          ...
          for (j = 0; j < m; ++j) {
            var o = links[j];
            neighbors[o.source.index_].push(o.target);
            neighbors[o.target.index_].push(o.source);
          }
        }
        return neighbors[i];
      }
  ...
  function d3_layout_forceAccumulate(quad, alpha, charges) {
    ...
    if (quad.point) {
      ...
      var k = alpha * charges[quad.point.index_];
