...
function create$1(node, id, self) {
  ...
  function start(elapsed) {
    ...
    for (i in schedules) {
      ...
      if (o.state === RUNNING) {
        o.state = ENDED;
        o.timer.stop();
        o.on.call("interrupt", node, node.__data__, o.index_, o.group);
        delete schedules[i];
      }
    ...
    self.state = STARTING;
    self.on.call("start", node, node.__data__, self.index_, self.group);
    ...
    for (i = 0, j = -1; i < n; ++i) {
      if (o = self.tween[i].value.call(node, node.__data__, self.index_, self.group)) {
        tween[++j] = o;
      }
    }
    tween.length = j + 1;
  ...
  function tick(elapsed) {
    ...
    if (self.state === ENDING) {
      self.on.call("end", node, node.__data__, self.index_, self.group);
      stop();
    }
  }
...
function interrupt(node, name) {
  ...
  for (i in schedules) {
    ...
    if (active) schedule$$1.on.call("interrupt", node, node.__data__, schedule$$1.index_, schedule$$1.group);
    delete schedules[i];
  }
...
function collide(radius) {
  ...
  function force() {
    ...
    for (var k = 0; k < iterations; ++k) {
      ...
      for (i = 0; i < n; ++i) {
        node = nodes[i];
        ri = radii[node.index_], ri2 = ri * ri;
        xi = node.x + node.vx;
        yi = node.y + node.vy;
        tree.visit(apply);
      }
  ...
  function prepare(quad) {
    if (quad.data) return quad.r = radii[quad.data.index_];
  ...
  function initialize() {
    ...
    radii = new Array(n);
    for (i = 0; i < n; ++i) node = nodes[i], radii[node.index_] = +radius(node, i, nodes);
  }
...
function link(links) {
  ...
  function defaultStrength(link) {
    return 1 / Math.min(count[link.source.index_], count[link.target.index_]);
  }
  ...
  function initialize() {
    ...
    for (i = 0, count = new Array(n); i < m; ++i) {
      ...
      count[link.source.index_] = (count[link.source.index_] || 0) + 1;
      count[link.target.index_] = (count[link.target.index_] || 0) + 1;
    }

    for (i = 0, bias = new Array(m); i < m; ++i) {
      link = links[i], bias[i] = count[link.source.index_] / (count[link.source.index_] + count[link.target.index_]);
    }
...
function manyBody() {
  ...
  function initialize() {
    ...
    for (i = 0; i < n; ++i) node = nodes[i], strengths[node.index_] = +strength(node, i, nodes);
  }
  ...
  function accumulate(quad) {
    ...
    // For leaf nodes, accumulate forces from coincident quadrants.
      ...
      q.x = q.data.x;
      q.y = q.data.y;
      do strength += strengths[q.data.index_];
      while (q = q.next);
    }
  ...
  function apply(quad, x1, _, x2) {
    ...
    do if (quad.data !== node) {
      w = strengths[quad.data.index_] * alpha / l;
      node.vx += x * w;
      node.vy += y * w;
...
function createEdge(left, right, v0, v1) {
  ...
  cells[left.index_].halfedges.push(index);
  cells[right.index_].halfedges.push(index);
  return edge;
}
  ...
  triangles: function() {
    ...
    this.cells.forEach(function(cell, i) {
      ...
      while (++j < m) {
        ...
        s1 = e1.left === site ? e1.right : e1.left;
        if (s0 && s1 && i < s0.index_ && i < s1.index_ && triangleArea(site, s0, s1) < 0) {
